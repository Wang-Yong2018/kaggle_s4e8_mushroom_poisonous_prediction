...
it is used car price prediction, evaluation metric is rmse. the mean price of train is 43878.02. use mean price as single model , the kaggle public/private score is 78643/70105.
...
train<- readr::read_csv(file.path(data_path, 'train.csv'),show_col_types = FALSE)
test <- readr::read_csv(file.path(data_path, 'test.csv'),show_col_types = FALSE)
...
```{r}
set.seed(1234)
df_split <- initial_split(train|>head(10**5), prop = 0.8, strata = class)
...
rcp_baseline <-
  ...
  step_mutate(class=as.factor(class),skip=T)|>
